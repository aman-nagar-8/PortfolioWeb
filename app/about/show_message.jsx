...
//   }, []);
  ...
  return (
    <div>
      <div>
        <button onClick={get_data} className="w-25 h-8 font-normal text-sm bg-blue-500 rounded-lg mb-5 cursor-pointer">
          See Messages
        </button>
      </div>
      <div className="w-[800px] flex flex-wrap gap-7" >
        ...
        ske_arr.map((_, index)=>(
         <div key={index} className=" w-90 h-28 bg-zinc-900 mr-5 p-3 rounded-xl">
          ...
          <div className="w-50 h-2 bg-zinc-700 rounded-2xl" >
          </div>
          </div> 
        )):
        all_mess.map((mess, index) => (
          ...
          <div className="w-7 h-7 rounded-lg bg-zinc-500"></div>
          <div>
          <h4 className="font-normal text-sm">{mess.name}</h4>
